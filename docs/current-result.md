...
The [published-flow compatibility check](nsc-flow-compatibility.md) now tests the next route to finite matching. The quoted generalized Palatini flow has a larger connection field space, and its metric contribution leaves the Dirac three-form coupling subspace in d=5. Its fermion contribution preserves that relation. This exact projection test does not compute an NSC flow: it requires the actual variables, constraint measure and derivative terms to be matched before borrowing a fixed point. The earlier bare spectral trace does not replace the later scale-integrated quantum definition of the common action.

The [five-dimensional UV match](nsc-torsion-uv-map.md) now supplies the leading proper-time bulk coefficient of the candidate interaction and its flat quadratic derivative correction. A matching window keeps this induced piece separate from the remaining one-loop determinant. Leading stiffness9 does not determine the full local ratio: the finite datum c_T-9c_R remains to be fixed by the common functional and boundary matching. The coefficient scale is not a physical torsion mass. This exact development record is covered by `make reproduce-development`.
